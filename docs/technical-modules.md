...
PMON — первый доменный модуль. Он скачивает `.pm` файлы с РРС по TFTP. Тип РРС берётся из поля `device` в metadata target. PMON хранится на РРС 7 дней, поэтому задача имеет `expires_at`. Scheduler умеет разворачивать `target_dates: last_7_days`, чтобы после простоя догонять пропущенные PMON-файлы.
